...
`go-ruby-parquet/parquet` reimplements Ruby's `parquet` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-parquet/parquet` directly, with no Ruby runtime;
